...
const WORKBOOK_MATCHERS = [
  { unit: "Aguas Claras", fileName: "Fechamento Aguas Claras_31-03.xlsx" },
  { unit: "Joquei Clube", fileName: "Jóquei  31-03.xlsx" },
  { unit: "Itororo", fileName: "Fechamento Itororó_31-03.xlsx" },
] as const;
...
type UnitName = (typeof WORKBOOK_MATCHERS)[number]["unit"];
type PaymentCategory = "avista" | "prazo";
...
export type DashboardData = {
  ...
  notes: string[];
  sourceFiles: Array<{ unit: UnitName; fileName: string }>;
  sales: SaleRecord[];
};
...
function readSheetRows(filePath: string, sheetName: string, headerRowIndex: number) {
  ...
  if (!sheet) {
    throw new Error(`A aba ${sheetName} nao foi encontrada em ${path.basename(filePath)}.`);
  }
...
function resolveWorkbookPath(unit: UnitName) {
  const matcher = WORKBOOK_MATCHERS.find((item) => item.unit === unit);

  if (!matcher) {
    throw new Error(`Matcher nao encontrado para ${unit}.`);
  }

  return path.join(DATA_DIR, matcher.fileName);
}
...
function normalizePayment(rawValue: unknown) {
  ...
  if (normalized === "A VISTA") {
    return { label: "A vista", category: "avista" as const };
  }
  if (normalized === "PIX") {
    return { label: "Pix", category: "avista" as const };
  }
...
function buildAguasClaras(asOfDate: Date): SaleRecord[] {
  const filePath = resolveWorkbookPath("Aguas Claras");
  const rows = readSheetRows(filePath, "Fechamento", 1);
...
function buildJoquei(asOfDate: Date): SaleRecord[] {
  const filePath = resolveWorkbookPath("Joquei Clube");
  const rows = readSheetRows(filePath, "Planilha2", 1);
  ...
  return rows
    ...
    .map((row) => {
      ...
      return {
        ...
        installmentsPaid,
        sourceFile: path.basename(filePath),
        notes: payment.label === "Misto" ? ["Vendas mistas foram classificadas como recebimento a vista."] : [],
      };
    });
}

function buildItororo(asOfDate: Date): SaleRecord[] {
  const filePath = resolveWorkbookPath("Itororo");
  const rows = readSheetRows(filePath, "Fechamento", 1);
  ...
  return rows
    ...
    .map((row) => {
      ...
      const installmentReceived =
        payment.category === "prazo"
          ? Math.min(expectedAmount, installmentsPaid * 49.9)
          : 0;
...
function buildDashboardData(): DashboardData {
  ...
  const sales = [...buildAguasClaras(asOfDate), ...buildJoquei(asOfDate), ...buildItororo(asOfDate)]
  ...
  return {
    generatedAt: BRAZILIAN_DATE.format(asOfDate),
    totals: {
      sales: totals.sales,
    ...
    unitBreakdown,
    notes,
    sourceFiles: WORKBOOK_MATCHERS.map((item) => ({
      unit: item.unit,
...
export function getDashboardData() {
  if (!dashboardCache) {
    dashboardCache = buildDashboardData();
  }
